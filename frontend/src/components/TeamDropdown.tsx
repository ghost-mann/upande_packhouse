...
export default function TeamDropdown({ value, onChange }: Props) {
  ...
  return (
    <div ref={ref} className="relative">
      <button
        onClick={() => setOpen((o) => !o)}
        className="glass-soft flex items-center gap-2 rounded-2xl px-3.5 py-1.5 text-[13px] font-medium text-ink transition hover:bg-white/70"
      >
            ...
            return (
              <button
                ...
                className={[
                  ...
                  active ? 'bg-accent-blue/15 font-semibold text-accent-blue' : 'text-ink-soft hover:bg-white/60',
                ].join(' ')}
              >
                {t === 'all' ? 'All Teams' : t}
