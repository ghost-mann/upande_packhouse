      ...
      {open && (
        <div className="glass-strong glass-sheen absolute z-50 mt-1 w-full overflow-hidden rounded-xl p-1.5 animate-fade-up">
          <input
            ...
            className="mb-1 w-full rounded-lg bg-gray-100 px-2.5 py-1.5 text-[12.5px] outline-none placeholder:text-ink-mute"
          />
            ...
            {filtered.length ? (
              filtered.map((o) => (
                <button
                  ...
                  className={['block w-full truncate rounded-lg px-2.5 py-1.5 text-left text-[12.5px] transition', o === value ? 'bg-accent-blue/15 font-semibold text-accent-blue' : 'text-ink-soft hover:bg-gray-100'].join(' ')}
                >
                  {o}
                </button>
              ))
