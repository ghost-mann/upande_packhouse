...
export default function VarietyModal({ opl, onClose }: { opl: Opl | null; onClose: () => void }) {
  ...
  return (
    <div
      onClick={onClose}
      className="fixed inset-0 z-[100] flex items-center justify-center bg-gray-900/30 p-5 backdrop-blur-sm animate-fade-up"
    >
      <div
      ...
      >
        <div className="flex items-center justify-between border-b border-line px-5 py-3.5">
          ...
          <button onClick={onClose} className="grid h-7 w-7 place-items-center rounded-full text-ink-mute transition hover:bg-gray-100 hover:text-ink">
            <X size={16} />
          </button>
        </div>
        <div className="overflow-y-auto px-5 py-2 no-scrollbar">
          {opl.varieties.map((v) => {
            ...
            return (
              <div key={v.name} className="border-b border-line py-2.5 last:border-0">
